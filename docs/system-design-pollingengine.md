...
contracts hundreds of other engineers build on, getting concurrency right, and making correctness
*the default* rather than something callers must remember.

`PollingEngine` is exactly that: an **in-process, multiplatform SDK** that repeatedly calls a
function until a condition is met or a limit is hit — reliably, cancellably, and without hammering
the backend.

This document walks through the design the way you'd present it in an interview:
...
The recurring theme: **a polling engine is a small surface area hiding a large amount of concurrency
and timing correctness.** The interesting part is never "call the API in a loop" — it's everything
that goes wrong around that loop.
...
> error we shouldn't retry, or we exhaust our time/attempt budget — safely, across Android and iOS
...
These share a shape: **client-driven long-polling**, where the client controls cadence and the
server has no push channel.

A second family is **continuous observation** rather than converge-then-stop: an always-on,
fixed-interval view of a remote resource (e.g. a live list) that emits every successful tick, fans
one fetch out to several independent UI consumers, and stops only when the resource drains or all
...
Stating non-goals is a staff-level signal: you're scoping the system instead of boiling the ocean.
...
2. **Coroutines as the concurrency substrate.** A gift and a constraint: structured concurrency,
   cooperative cancellation, and `suspend` functions for free — but we must respect their rules
   precisely (especially around `CancellationException`).

3. **Library, not application.** We don't own the process — no `main()`, Android `Application`, or
   iOS app delegate to assume. State we keep is *process-global* and must be defensively managed.

```mermaid
flowchart LR
    subgraph commonMain["commonMain (shared)"]
        A[Public API<br/>Polling facade + PollBuilder DSL<br/>+ PollHandle + SharedPoll]
...
A staff engineer designs the **boundary** before the implementation — it's what you can never take
back.

### 3.1 The fluent facade
...
public object Polling {
    ...
    public suspend fun activeIds(): List<String>
    public suspend fun cancelAll()
    public suspend fun shutdown()
}

public class PollBuilder<T> {
    // refinements (chainable, all optional): until · stopWhen · stopWhenResult · every · backoff
    //   · atMost · timeout · timeoutPerAttempt · retryWhen · mapErrors · on · onAttempt · onResult
    //   · onComplete · keepAliveFor · replayLast
    public fun start(scope: CoroutineScope): PollHandle<T>                 // converge, controllable
    public suspend fun await(): PollingOutcome<T>                          // converge, one-shot
    public fun collect(scope: CoroutineScope, onValue: suspend (T) -> Unit): PollHandle<T> // observe
    public fun asFlow(): Flow<T>                                           // observe, cold flow
    public suspend fun shared(key: Any): SharedPoll<T>                     // multiplexed
}
```

Two deliberate choices here:

- **One entry, the engine stays `internal`.** Every poll begins at `Polling.poll { … }`; the engine
  — with all its mutable state and coroutine machinery — plus `PollingConfig`/`BackoffPolicy` are
  hidden behind `internal` visibility and `explicitApi()` enforcement. The public surface is a small,
  fluent DSL; the volatile implementation can't leak.

- **The terminal verb picks the run model — the caller never chooses upfront.** A `PollBuilder` is a
  neutral description; *how* it runs is decided by the verb you finish with. The first two *converge*
  to a single `PollingOutcome`; the rest are *continuous*:
    - `.start(scope) : PollHandle<T>` — a *managed* converge session whose `outcomes` flow emits
      exactly one terminal outcome. The handle carries the id and control (pause/resume/cancel/retune)
      synchronously, so callers never resolve an id after starting (contrast the old `startPolling`,
      which forced an id lookup — see §6.4).
    - `.await() : PollingOutcome<T>` — a *fire-and-await* one-shot that runs in the caller's own
      coroutine.
    - `Polling.sequence(vararg)` — sequential pipelines that short-circuit on the first non-success.
    - `.collect(scope) { } : PollHandle<T>` / `.asFlow() : Flow<T>` — a **continuous stream** of the
      value of *every* successful tick, running until a stop condition fires (terminal success via
      `until`, `stopWhen`, a non-retryable failure, or attempt/overall limits). Pair with an
      unbounded cadence for an always-on observer (see §6.7).
    - `.shared(key) : SharedPoll<T>` — a **multiplexed** session keyed by `key`: one poll loop, one
      fetch per tick, fanned out to many subscribers, each with its own filter, with a
      subscriber-driven `WhileSubscribed` lifecycle (see §6.8).

The split is the key contract decision: a **converge** verb answers "what was the final result?",
while a **continuous** verb answers "feed me every value as it arrives." Serving both from one verb
would force one caller to fight the type signature.
...
`viewModelScope` tearing down on screen exit), `awaitClose` fires and cancels the polling job. The
collector never has to remember to clean up — *structured concurrency does it.* Intermediate
progress goes through the observability hooks (§7), not `Flow` emissions, keeping the stream's type
honest: it carries the *result*, not the *journey*.

It's a genuine trade-off: a single-emission `outcomes` flow is unusual, and an alternative would
stream every value and let the caller fold them. The converge verbs (`.start`/`.await`) deliberately
optimize for "give me the answer, tie cleanup to my lifecycle." The *other* need — "feed me every
value as it arrives" — is served by the continuous verbs (`.collect`/`.asFlow`/`.shared`, §6.7–6.8)
rather than by overloading this stream: same `Flow` machinery and `awaitClose` discipline, different
cardinality and element type (`T`, not `PollingOutcome<T>`).

### 3.3 The builder — making the right thing easy

The poll is built fluently, so the common path reads like a specification (and a plain `poll { }`
fetch returns a value directly — no `PollingResult` wrapping needed):

```kotlin
val outcome = Polling.poll { api.getJobStatus(jobId) }   // throw on error
    .until { it == Status.DONE }
...
Every refinement has a safe default — "make illegal states unrepresentable" applied at the
ergonomics layer:

- `until` is optional; omit it and the poll runs as a stream until `stopWhen`/a limit fires, which
  is exactly what the continuous verbs want. A converge verb without `until` simply has no success
  condition — it stops on `stopWhen`, exhaustion, or timeout.
- `every` vs `backoff { }` are mutually-exclusive ways to express cadence (the latter wins if both
  are set); `atMost`/`timeout`/`timeoutPerAttempt` layer caps on top, order-independently.
- `keepAliveFor`/`replayLast` only bite in `.shared(key)` mode; they are no-ops for the other verbs —
  documented rather than type-enforced, a minor ergonomics-vs-strictness trade.
...
This is what the user's `fetch` returns *each time*. `Success` is not necessarily terminal —
`Waiting` explicitly models "the call succeeded and told me to keep waiting," the single most
important domain distinction in long-polling.
...
An interviewer will poke at this. The answer: **they live at different layers and change for
different reasons.** `PollingResult` is the *input contract* (what the user's fetch speaks);
`PollingOutcome` is the *output contract* (what the engine concludes). Collapsing them would force
the user to understand engine-level concepts like "exhausted" and "timeout," and make the fetch
signature lie about what it can return. Separation keeps each contract honest and independently
evolvable.
...
- Callers map thrown exceptions to `Error` via `.mapErrors { … }`.
...
- Stock retry predicates (`Retry.networkOrServer`, `Retry.always`, `Retry.never`) are provided as
  *optional convenience* for `.retryWhen`, not as coupling.

The principle: **a library must not drag the caller's transport/serialization choices into its
core.** That decoupling is what keeps the dependency footprint at zero.
...
```mermaid
flowchart TD
    Caller -->|poll { } .start / .await / .collect / .asFlow / .shared / sequence| Facade[Polling facade]
    ...
    Control[PollHandle: pause/resume/cancel/retune] --> Registry
```
...
- **`SupervisorJob`** so one failing/cancelled session does **not** cancel its siblings. With a
  regular `Job`, a single child failure propagates up and kills every active poll — unacceptable for
  a shared engine.
- **`Dispatchers.Default`** as the baseline (CPU-bound coordination), with each session free to
...
- **A `Mutex`, not `synchronized`.** `synchronized` is a JVM blocking primitive and isn't
  multiplatform; a coroutine `Mutex` suspends instead of blocking and compiles on native. It guards
  the three registries (`active`, `controls`, `sharedSessions`) — the only shared mutable state —
  with tiny critical sections (map insert/remove/read).
- **A `sharedSessions` registry keyed by `Any`** backs the multiplexing in `shared(key, …)`:
  `getOrPut(key)` under the mutex guarantees concurrent callers for the same key get the *same* live
  session rather than racing to create duplicates (§6.8). `shutdown()` clears it so a shut-down
  engine holds no dangling sessions.

> **Critique worth raising yourself:** a process-global singleton is convenient but introduces
> global mutable state — a shared resource across the whole app — and `shutdown()` is effectively
> irreversible (it cancels the supervisor and sets `isShutdown`, with no `restart()` to rebuild the
> scope). A more testable alternative is an *instantiable* engine injected where needed. The
> singleton trades testability/isolation for call-site convenience — reasonable for an SDK, but know
> its cost.
...
Each session has a `Control` holding two `StateFlow`s. The elegant part: **control is communicated
through observable state, not interrupts.**

- **Pause/resume** flips `state`. While sleeping, the loop *suspends on the flow* until it reads
...
- **Dynamic backoff** publishes a new `BackoffPolicy` into `control.backoff`; the loop re-reads it
  at the top of every iteration, so policy changes take effect next cycle without restarting the
  session.
...
the final outcome escapes. This avoids forking the most correctness-sensitive code in the module
into two copies that could drift. The loop also bakes in unbounded-policy handling (§6.7):
attempt-limit and overall-timeout checks are skipped when the policy declares them disabled.
...
exponential backoff has a fatal flaw at scale: if 10,000 clients hit an outage at the same instant,
they all back off by *the same* amount and retry *in the same instant* — re-creating the spike. This
is the **thundering herd / retry storm** problem.
...
hours. Capping converts late-stage behavior into steady-state polling at a sane ceiling (default
30s).

> **Trade-off to mention:** AWS's well-known guidance favors *decorrelated jitter*
> (`sleep = min(cap, random(base, prev·3))`), which disperses retries even more aggressively. The
...
A naive implementation would use only the per-attempt timeout — and could start a 10s attempt with
only 2s of overall budget left, blowing past the deadline by 8s. Taking the **minimum** of the
per-attempt timeout and the *remaining* overall budget guarantees no single attempt overruns the
global deadline. The same clamp applies to the **sleep** between attempts
...
— *ran out of time* (`Timeout`) versus *ran out of attempts* (`Exhausted`). Callers often treat
these differently (retry later vs. surface a hard error), so the engine refuses to conflate them.

### 6.4 Cancellation correctness — the "rogue `CancellationException`" problem

The single most interview-worthy detail in the whole module.

In coroutines, `CancellationException` is **control flow, not an error**. The cardinal rule is
*never swallow it* — catching `Throwable` and continuing makes a coroutine uncancellable. But
there's a nasty wrinkle: some libraries and I/O stacks **throw `CancellationException` as a normal
failure** even when *your* coroutine wasn't cancelled. Blindly rethrowing every one lets a
misbehaving dependency tear down a perfectly healthy poll.
...
This correctness detail never shows up in a demo but absolutely shows up in production crash logs.
Explaining *why* `isActive` is the right discriminator is a strong staff-level signal.
...
*suspends* on the flow (zero CPU) instead of spinning.
...
> "freeze the clock." That's either correct (a deadline is a deadline) or surprising (the user
> expected pause to suspend the SLA too), depending on intent. Name the ambiguity rather than
...
A small **railway-oriented pipeline**: stages run in sequence, each must reach `Success` to proceed,
and the first non-success ends the chain (returning the failing outcome). It composes multi-step
workflows — e.g., "wait for upload to finish, *then* wait for processing to finish" — without the
caller hand-rolling nested polling.
...
to *skip* the corresponding checks. The point is **backward compatibility through sentinel +
default**: existing callers pass nothing, get the old positive defaults (8 attempts, 120s), and
behave identically; only a caller who explicitly opts into `0` unlocks unbounded mode. No new type,
no breaking change.
...
`shared`. Callers can still bound it with explicit limits. Encoding the intent in a factory is the
same "capture tuned configurations so callers don't re-derive the math" principle as `quick20s`,
applied to constant cadence.
...
It completes only when a terminal fires — terminal success, `stopWhen`, a non-retryable failure, or
(if the policy is bounded) attempt/overall limits. Retryable per-tick errors are surfaced through
...
own filter, must collectively cause **exactly one** `fetch()` per tick. It's a single `observe`
upstream multiplexed with `shareIn`:

```kotlin
private class SharedSessionImpl<T>(...) : SharedPoll<T> {
...
- **One fetch per tick regardless of subscriber count.** `shareIn` runs a *single* collection of the
  upstream; every `stream()` / `stream(filter)` view reads from the same `SharedFlow`. A `filter`
  applies *downstream* of the shared emission, so adding a filtered view never triggers another
  network call.
- **Subscriber-driven lifecycle.** `WhileSubscribed(stopTimeoutMillis)` starts the poll on the first
  subscriber and stops it `stopTimeoutMs` after the last leaves — then restarts on the next
  subscriber. The grace period avoids thrashing the upstream during brief gaps (e.g. a screen
  rotation).
...
independent one. The canonical "one upstream, many views" pattern — e.g. an activations view and an
associations view over the same VIN, both fed by one 10s network call.
...
- **Structured by construction.** Every callback hands you typed values (attempt index, delay,
  result, outcome), not pre-formatted strings, so you can route them to Logcat, `os_log`, metrics
  counters, or traces as you see fit.

The SDK-appropriate answer to "how do I get logging/metrics in a library without forcing a logging
framework on my users?" — expose typed seams, ship adapters separately.
...
The test taxonomy maps one-to-one onto the §1.2 requirements table — that traceability is itself a
design artifact, now extended to cover the streaming/shared requirements.
...
The point isn't pattern-bingo — each pattern resolves a specific force (extensibility, safety,
decoupling) the requirements created.
...
2. **Two `PollingConfigBuilder` classes.** A property-based builder in `polling/builder/` (the
   `startPolling { }` facade DSL) and a separate strategy-based fluent builder in `polling/` (the
   `pollingConfig { }` entry point). Two ways to build the same `PollingConfig` invite drift — a
   real smell. I'd consolidate to one.
...
understand the design space it lives in."
...
devices?" Extensions worth discussing:
...
and **how cooperatively** clients behave — moving the interesting decisions to the policy layer,
exactly where this design already put its seams.
...
concurrency, timing, and cancellation — and a good design makes the *correct* behavior the
*default*, so the thousands of engineers calling `Polling.run { … }` never have to think about any
of it.
...
| Shared/streaming session contract       | `polling/SharedPoll.kt`                                                                             |
