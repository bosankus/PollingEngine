...
class PollingStreamingTest {
    ...
    @Test
    fun f1_unboundedRun_runsPastDefaultAttemptCap() =
        runTest {
            var calls = 0
            val config =
                PollingConfig(
    ...
    @Test
    fun f2_fixedInterval_hasConstantCadence() =
        runTest {
    ...
    @Test
    fun f3_observe_emitsEverySuccessTick() =
        runTest {
            var n = 0
            val config =
                PollingConfig(
    ...
    @Test
    fun f4_observe_stopsOnEmptyWithoutEmitting() =
        runTest {
            var tick = 0
            val config =
                PollingConfig<List<Int>>(
                    fetch = {
                        tick++
    ...
    @Test
    fun f4_run_stopWhenEmpty_yieldsExhausted() =
        runTest {
            var tick = 0
            val config =
                PollingConfig<List<Int>>(
    ...
    @Test
    fun f5_shared_singleFetchPerTick_distinctFilters() =
        runTest {
    ...
    @Test
    fun f6_shared_startsOnSubscribe_stopsAfterGrace() =
        runTest {
            PollingEngine.installScopeForTesting(backgroundScope)
            var fetches = 0
            val config =
    ...
    private fun unboundedNoDelay(): BackoffPolicy =
        BackoffPolicy(
            initialDelayMs = 0,
            maxDelayMs = 1,
            multiplier = 1.0,
