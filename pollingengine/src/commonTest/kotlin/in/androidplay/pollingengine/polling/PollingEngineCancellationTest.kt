...
class PollingEngineCancellationTest {
    @Test
    fun rogueCancellationException_isTreatedAsFailureAndLeadsToExhausted() =
        runTest {
            val config =
                PollingConfig<Unit>(
                    fetch = {
            ...
            assertTrue(
                outcome is PollingOutcome.Exhausted,
                "Expected Exhausted outcome when CancellationException is thrown under active scope, but was: $outcome",
            )
        }
}
