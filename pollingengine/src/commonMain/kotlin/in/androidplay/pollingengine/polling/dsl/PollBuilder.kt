...
public class PollBuilder<T> internal constructor(
    ...
    public fun stopWhen(predicate: (T) -> Boolean): PollBuilder<T> =
        apply {
    ...
    public fun stopWhenResult(predicate: (PollingResult<T>) -> Boolean): PollBuilder<T> = apply { stopWhen = predicate }

    // --- cadence ---

    /** Poll at a constant cadence of [interval] between attempts (no growth, no jitter). */
    public fun every(interval: Duration): PollBuilder<T> = apply { intervalMs = interval.inWholeMilliseconds }

    /** Use exponential backoff with jitter; see [BackoffSpec]. Overrides [every]. */
    public fun backoff(spec: BackoffSpec.() -> Unit): PollBuilder<T> = apply { explicitBackoff = BackoffSpec().apply(spec).toPolicy() }
    ...
    public fun timeoutPerAttempt(budget: Duration): PollBuilder<T> = apply { perAttemptTimeout = budget }

    // --- error handling ---

    /** Decide whether to keep polling after a failed attempt. See the [Retry] presets. */
    public fun retryWhen(predicate: (Error?) -> Boolean): PollBuilder<T> = apply { retryWhen = predicate }

    /** Map a thrown exception into a domain [Error] for [retryWhen] and reporting. */
    public fun mapErrors(mapper: (Throwable) -> Error): PollBuilder<T> = apply { mapErrors = mapper }

    /** Run the poll loop on a specific dispatcher. Defaults to [Dispatchers.Default]. */
    public fun on(dispatcher: CoroutineDispatcher): PollBuilder<T> = apply { this.dispatcher = dispatcher }

    // --- observability ---

    /** Called before each attempt with the attempt index and the upcoming delay (0 for immediate). */
    public fun onAttempt(hook: (attempt: Int, delayMs: Long?) -> Unit): PollBuilder<T> = apply { onAttempt = hook }

    /** Called after each attempt with its [PollingResult]. */
    public fun onResult(hook: (attempt: Int, result: PollingResult<T>) -> Unit): PollBuilder<T> = apply { onResult = hook }

    /** Called once with the terminal outcome, total attempts, and elapsed time. */
    public fun onComplete(hook: (attempts: Int, durationMs: Long, outcome: PollingOutcome<T>) -> Unit): PollBuilder<T> =
        apply { onComplete = hook }
     ...
     */
    public fun start(scope: CoroutineScope): PollHandle<T> = EngineSessionHandle(PollingEngine.launch(scope, buildConfig()))
    ...
    ): PollHandle<T> = EngineSessionHandle(PollingEngine.launch(scope, buildConfig(), onValue))
    ...
    public suspend fun shared(key: Any): SharedPoll<T> =
        PollingEngine.shared(
            key = key,
            config = buildConfig(),
    ...
    internal fun buildConfig(): PollingConfig<T> =
        PollingConfig(
            fetch = fetch,
